...
# Options that should only be defined when in production
if PRODUCTION:
    # (security.W004) If your entire site is served only over SSL,
...
SECURE_CONTENT_TYPE_NOSNIFF = config('OF_SECURE_CONTENT_TYPE_NOSNIFF', default=PRODUCTION, cast=bool)
...
SECURE_BROWSER_XSS_FILTER = config('OF_SECURE_BROWSER_XSS_FILTER', default=PRODUCTION, cast=bool)

# (security.W008) Automatically return a 301 to the https:// version of the site
SECURE_SSL_REDIRECT = config('OF_SECURE_SSL_REDIRECT', default=PRODUCTION, cast=bool)

# (security.W012) Using a secure-only session cookie makes it more difficult for network traffic
# sniffers to hijack user sessions.
SESSION_COOKIE_SECURE = config('OF_SESSION_COOKIE_SECURE', default=PRODUCTION, cast=bool)

# (security.W016) Using a secure-only CSRF cookie makes it more difficult for network traffic
# sniffers to steal the CSRF token.
CSRF_COOKIE_SECURE = config('OF_CSRF_COOKIE_SECURE', default=PRODUCTION, cast=bool)
...
X_FRAME_OPTIONS = config('OF_X_FRAME_OPTIONS', default='DENY' if PRODUCTION else 'SAMEORIGIN')
