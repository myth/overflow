...
sed -i "s,myth/overflow.*,myth/overflow:v$version," docker-compose.yml
sed -i "/^version = /s/.*/version = \"$version\"/" pyproject.toml
uv sync
git commit -am "chore: Prepare v${version}"
echo "Pinned docker-compose.yml to myth/overflow:v$version and updated version in pyproject.toml"
